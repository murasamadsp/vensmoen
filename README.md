...
- Норвезька (`nb.json`) — структурний еталон; зміст можна редагувати в
  будь-якій мові через CMS.
...
Pages CMS показує редактору всі мови (`src/i18n/<код>.json`) та `Driftsdata`
(`src/data/site-settings.json`). GitHub Actions дивиться diff: мова, яку
змінили в CMS, стає джерелом для цього поля, і зміна перекладається на решту
мов. Якщо одночасно змінено кілька мов для одного поля, пріоритет має `nb`.
